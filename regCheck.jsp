...
<body><section><h2>JSPform</h2>
	...
	<h2>비밀번호:<%=request.getParameter("stud_passwd")%>
	<h2>성별/취미</h2>
	<h2>성별:<%=request.getParameter("stud_gender")%>, 취미:<% String [] uHobby=request.getParameter("stud_hobby")%>
		<%
			if(uHobby == null) { out.ptintln("입력내용없음");
			} else {
				for(int i = 0; i<uHobby.length;i++){
					out.ptintln(uHobby[i]);
				}
			}
		%>
	<br/></h2>
